...
DIR=$PWD/build/lib/libxcb-cursor/$1
if [[ ! -d $DIR ]]; then
  cmake --preset test-$1 -DCMAKE_INSTALL_PREFIX=/usr/local
  cmake --build build/test/$1 -j$(nproc) -t Xau xcb xcb-render-util xcb-image
  ...
  cmake --install build/test/$1 --component xcb-render-util
  cmake --install build/test/$1 --component xcb-image

  git clone https://gitlab.freedesktop.org/xorg/lib/libxcb-cursor.git --depth=1 --branch=xcb-util-cursor-0.1.5 $DIR
...
cmake --build build/test/$1 -j$(nproc) -t xcb-cursor
cmake --install build/test/$1 --component xcb-cursor
