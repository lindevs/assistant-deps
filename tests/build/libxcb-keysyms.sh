...
DIR=$PWD/build/lib/libxcb-keysyms/$1
if [[ ! -d $DIR ]]; then
  ...
  git clone https://gitlab.freedesktop.org/xorg/lib/libxcb-keysyms.git --depth=1 --branch=xcb-util-keysyms-0.4.1 $DIR
...
cmake --build build/test/$1 -j$(nproc) -t xcb-keysyms
cmake --install build/test/$1 --component xcb-keysyms
