...
if [[ ! -d $DIR ]]; then
  ...
  git clone https://gitlab.freedesktop.org/xorg/lib/libxcb-wm.git --depth=1 --branch=xcb-util-wm-0.4.2 $DIR
...
cmake --build build/test/$1 -j$(nproc) -t xcb-ewmh xcb-icccm
cmake --install build/test/$1 --component xcb-wm
