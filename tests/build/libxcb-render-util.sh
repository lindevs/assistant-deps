...
DIR=$PWD/build/lib/libxcb-render-util/$1
if [[ ! -d $DIR ]]; then
  ...
  git clone https://gitlab.freedesktop.org/xorg/lib/libxcb-render-util.git --depth=1 --branch=xcb-util-renderutil-0.3.10 $DIR
...
cmake --build build/test/$1 -j$(nproc) -t xcb-render-util
